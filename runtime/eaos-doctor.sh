...
if [ -f "$CLAUDE_DIR/settings.json" ] && grep -q "eaos-hook.sh" "$CLAUDE_DIR/settings.json" 2>/dev/null; then
  ...
  grep -q "eaos-hook.sh.* posttool" "$CLAUDE_DIR/settings.json" 2>/dev/null || \
    note "no PostToolUse entry — re-run ./runtime/install-eaos-hooks.sh so each session binds to ITS task"
else
  note "hooks not wired (optional) — opt in with: ./runtime/install-eaos-hooks.sh"
fi
...
if command -v python3 >/dev/null 2>&1; then
  if python3 "$EAOS_DIR/tests/validate-eaos.py" >/dev/null 2>&1; then pass "validate-eaos.py: repo internally consistent"; else
    bad "validate-eaos.py reported errors — run: python3 tests/validate-eaos.py"; fi
else
