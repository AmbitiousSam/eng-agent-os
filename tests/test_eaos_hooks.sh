#!/usr/bin/env bash
# test_eaos_hooks.sh — bash tests for runtime/eaos-hook.sh (M-007 hooks-as-accelerators)
# and runtime/install-eaos-hooks.sh. Feeds crafted hook-JSON on stdin against a real
...
# eaos-hook.sh always falls back to THIS checkout's runtime/eaos — never a real
