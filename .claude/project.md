...
python3-stdlib tooling (eaos/runtime/eaos runtime CLI, validate-eaos.py, eval_check.py,
...
- **Binding exit codes:** the `eaos` CLI (eaos/runtime/eaos → ~/.claude/eaos/bin/eaos) owns task
...
- **Honest portability:** Claude Code = full team (isolated subagents, tiers, fresh verifier).
  Other IDEs = eaos/adapters/solo-mode.md (grounded single agent + second-session verification).
